...
/**
 * Created by Олександр on 14.12.2017.
 */
...
//E0DCC8
    ...
    public String convertToHex(int intColor) {
        ...
        String red = "" + Integer.valueOf(hexColor.substring(1, 3), 16);
        String green = "" + Integer.valueOf(hexColor.substring(3, 5), 16);
        String blue = "" + Integer.valueOf(hexColor.substring(5, 7), 16);

        String line = "";

        for (int i = 0; i < 3 - red.length(); i++) {
            line = line + "0";
        }
        line = line + red;

        for (int i = 0; i < 3 - green.length(); i++) {
            line = line + "0";
        }
        line = line + green;

        for (int i = 0; i < 3 - blue.length(); i++) {
            line = line + "0";
        }
        line = line + blue;
    ...
    public float[] converterColorToHSV(int color) {
    ...
    public String getTextForView(int color){
        String fullHex= Integer.toHexString(color).toUpperCase();
        String hex = "#" + fullHex.substring(2, fullHex.length());
        return hex;
    }
}